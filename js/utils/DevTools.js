...
export default class DevTools {
    ...
    _setupPostMessageListener() {
        window.addEventListener('message', (event) => {
            ...
            if (data.type === 'SET_CATEGORY') {
                import('../utils/EventEmitter.js').then(module => {
                    const EventManager = module.default;
                    EventManager.emit('stream:categoryUpdated', data.category);
                    console.log(`🛠️ DevTools: Category manually set to: ${data.category}`);
                });
            }
        });
    }
